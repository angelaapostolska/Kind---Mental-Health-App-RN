...
import { setHideOnboarding, setUserName } from '@/store/commonSlices/appSlice';

const SETTINGS = [
  { icon: 'notifications', label: 'Notifications', desc: 'Reminders & alerts', iconLib: 'material' },
  { icon: 'brightness-6', label: 'Appearance', desc: 'Theme & display', iconLib: 'material' },
  { icon: 'volume-up', label: 'Sound & Haptics', desc: 'Audio preferences', iconLib: 'material' },
  { icon: 'security', label: 'Privacy', desc: 'Data & security', iconLib: 'material' },
];
    ...
    Alert.alert('Sign Out', 'Are you sure you want to sign out?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          dispatch(setHideOnboarding(false));
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.weekStats}>
          {[
            { icon: 'emoticon-happy-outline', label: 'Moods logged', val: 5, lib: 'mci' },
            { icon: 'book-open-variant', label: 'Journal entries', val: 2, lib: 'mci' },
            { icon: 'weather-windy', label: 'Breathe sessions', val: 3, lib: 'mci' },
          ].map((s) => (
      ...
      <View style={styles.card}>
        {SETTINGS.map((item, i) => (
          <TouchableOpacity key={item.label} style={[styles.settingRow, i < SETTINGS.length - 1 && styles.settingDivider]}>
...
const styles = StyleSheet.create({
  ...
  statCard: {
    flex: 1, backgroundColor: theme.colors.surface.one, borderRadius: 16, padding: theme.spacing.md,
  ...
  card: {
    backgroundColor: theme.colors.surface.one, borderRadius: 20, padding: theme.spacing.md,
    marginBottom: theme.spacing.md, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, elevation: 1,
  },
  sectionTitle: { fontSize: theme.typography.fontSize.paragraph.md, fontWeight: '700', color: theme.colors.text.primary, marginBottom: theme.spacing.sm },
  weekStats: { gap: 8 },
  weekStatRow: { flexDirection: 'row', alignItems: 'center', gap: theme.spacing.sm, paddingVertical: 4 },
  weekStatIcon: { width: 32, height: 32, borderRadius: 10, backgroundColor: theme.colors.surface.brandPrimary, alignItems: 'center', justifyContent: 'center' },
  ...
  settingIcon: { width: 36, height: 36, borderRadius: 10, backgroundColor: theme.colors.surface.brandPrimary, alignItems: 'center', justifyContent: 'center' },
  ...
  signOutBtn: {
    ...
    backgroundColor: theme.colors.surface.error, borderRadius: 16, padding: theme.spacing.md,
